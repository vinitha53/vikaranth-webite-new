...
export default async function ProductPage({ params }) {
  ...
  const heroLogo = brandLogos[product.brand] || productPartners[0]?.logo || "/logo-vikranth.webp";
  const heroLogoName = product.brand || productPartners[0]?.name || "Vikranth Chemical Corporation";
  ...
  return <main className={styles.page} data-product-page data-whatsapp-number={whatsappNumber}>
    ...
    <section className={styles.hero} data-hero><div className={styles.heroGlow} data-depth /><div className={styles.wrap}>
      ...
      </div><div className={styles.visualColumn}><div className={styles.productVisual} data-product-stage><span className={styles.visualWord} aria-hidden="true">{product.name}</span><img data-product-image src={product.image} alt={product.name} width="900" height="900" loading="eager" fetchPriority="high" decoding="async" /><span className={`${styles.productHeroLogo} ${product.brand === "Cacao Barry" ? styles.compactBarryLogo : ""} ${productPartners[0]?.slug === "nitta-gelatin-india-ltd" ? styles.largeNittaLogo : ""} ${productPartners[0]?.slug === "roquette" ? styles.darkRoquetteLogo : ""} ${productPartners[0]?.slug === "anchor" || product.brand === "Anchor" ? styles.cleanAnchorLogo : ""} ${productPartners[0]?.slug === "gujarat-ambuja-exports-ltd" ? styles.wideAmbujaLogo : ""}`}><img src={heroLogo} alt={`${heroLogoName} logo`} width="200" height="76" /></span><span className={styles.bulkBadge}><PackageCheck /> Bulk enquiry</span><div className={styles.imageLabel}><FlaskConical /><small>Commercial sourcing</small><strong>{product.name}</strong></div></div>
      {!['Vital Wheat Gluten', 'Skimmed Milk Powder'].includes(product.name) && <div className={`${styles.partnerPanel} ${showProductBrand ? styles.partnerPanelWithBrand : ""}`} data-partner-badge><span>{showProductBrand ? "Product brand & supplier" : productPartners.length ? "Verified product partner" : "Sourcing contact"}</span><div className={styles.partnerLogos}>
