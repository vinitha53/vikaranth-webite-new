...
export default async function PartnerPage({ params }) {
  ...
  return (
    <main className={styles.page} data-associate-page>
      ...
      <section className={`${styles.section} ${styles.sectionCream}`} id="about"><div className={styles.wrap}><div className={styles.aboutFeatureGrid}><div className={styles.aboutFeatureCopy} data-associate-reveal="left"><span className={styles.eyebrow}>About the supplier</span><h2>Ingredients engineered for the right application.</h2><p>{content.about}</p><div className={styles.aboutSupplierLogo}><img src={partner.logo} alt={`${partner.name} logo`} width="240" height="100" /></div><a className={styles.aboutProfileLink} href="#products">View supplier profile <ArrowRight /></a></div><div className={styles.applicationFeatureGrid} data-associate-stagger>{productLinks.slice(0,4).map((product)=><Link className={styles.applicationFeatureCard} href={`/products/${product.slug}/`} key={product.slug}><img src={product.image} alt={`${product.displayName || product.name} supplied by ${partner.name}`} width="640" height="480" loading="lazy"/><div><h3>{product.displayName || product.name}</h3><p>{product.usageCategory || product.category}</p></div></Link>)}</div></div><div className={styles.aboutAssurance} data-associate-reveal="up"><ShieldCheck/><span>Grade, source, specification and documentation are confirmed before quotation.</span><a href="#enquiry">Discuss your application <ArrowRight /></a></div></div></section>
