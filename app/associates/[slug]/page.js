...
export default async function PartnerPage({ params }) {
  ...
  return (
    <main className={styles.page} data-associate-page>
        ...
        <div className={styles.wrap}>
          ...
          <div className={styles.heroGrid}>
            ...
            <div className={styles.visual} data-associate-reveal="right">
              <div className={styles.visualFrame}><img src={partner.image} alt={`${partner.name} ${content.category.toLowerCase()} for professional applications`} width="900" height="700" fetchPriority="high" data-associate-hero-image /><div className={styles.visualTag}><small>Supplier portfolio</small><strong>{content.category}</strong></div></div>
              <div className={styles.logoPlate}><img src={partner.logo} alt={`${partner.name} logo`} width="240" height="100" /></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className={`${styles.section} ${styles.sectionCream}`} id="about"><div className={styles.wrap}><div className={styles.aboutFeatureGrid}><div className={styles.aboutFeatureCopy} data-associate-reveal="left"><span className={styles.eyebrow}>About the supplier</span><h2>Ingredients engineered for the right application.</h2><p>{content.about}</p><div className={styles.aboutSupplierLogo}><img src={partner.logo} alt={`${partner.name} logo`} width="240" height="100" /></div><a className={styles.aboutProfileLink} href="#products">View supplier profile <ArrowRight /></a></div><div className={styles.applicationFeatureGrid} data-associate-stagger>{content.applications.slice(0,4).map((application,index)=>{const applicationImage=Object.values(partner.productImages||{})[index]||partner.image;return <article className={styles.applicationFeatureCard} key={application}><img src={applicationImage} alt={`${partner.name} ${application}`} width="640" height="480" loading="lazy"/><div><h3>{application}</h3><p>{content.category} for professional applications.</p></div></article>})}</div></div><div className={styles.aboutAssurance} data-associate-reveal="up"><ShieldCheck/><span>Grade, source, specification and documentation are confirmed before quotation.</span><a href="#enquiry">Discuss your application <ArrowRight /></a></div></div></section>
