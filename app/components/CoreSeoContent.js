...
export default function CoreSeoContent({ content, compact = false, hideFaq = false, hideOverview = false, hideProcess = false }) {
  return <section className={`${styles.wrap} ${compact ? styles.compact : ""}`} aria-labelledby={`core-content-${content.title.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}`}>
    ...
    {!hideProcess && <div className={styles.process}><div><small>Procurement pathway</small><h2>How to move from enquiry to approval</h2></div><ol>{content.steps.map((step) => <li key={step}><Check aria-hidden="true"/><span>{step}</span></li>)}</ol></div>}
