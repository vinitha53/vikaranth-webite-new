...
export default function DetailHeaderClient() {
  ...
  useEffect(() => {
    ...
    const escape = (event) => { if (event.key === "Escape") { setMegaOpen(false); setIndustryMegaOpen(false); setSupplierMegaOpen(false); } };
  ...
  return <>
    ...
    <header className={styles.header} ref={headerRef}><div className={styles.headerInner}>
      ...
      <nav className={open ? styles.navOpen : ""} aria-label="Primary navigation"><Link href="/" onClick={closeMenus}>Home</Link><Link href="/about" onClick={closeMenus}>About</Link><button className={styles.productsTrigger} onClick={() => { setMegaOpen(value => !value); setIndustryMegaOpen(false); setSupplierMegaOpen(false); }} aria-expanded={megaOpen} aria-controls="site-products-mega-menu">Products <ChevronDown/></button><Link className={styles.mobileProductsLink} href="/products" onClick={closeMenus}>View all products</Link><button className={styles.productsTrigger} onClick={() => { setIndustryMegaOpen(value => !value); setMegaOpen(false); setSupplierMegaOpen(false); }} aria-expanded={industryMegaOpen} aria-controls="site-industries-mega-menu">Industries <ChevronDown/></button><Link className={styles.mobileProductsLink} href="/industries" onClick={closeMenus}>View all industries</Link><button className={styles.productsTrigger} onClick={() => { setSupplierMegaOpen(value => !value); setMegaOpen(false); setIndustryMegaOpen(false); }} aria-expanded={supplierMegaOpen} aria-controls="site-suppliers-mega-menu">Suppliers <ChevronDown/></button><Link className={styles.mobileProductsLink} href="/associates" onClick={closeMenus}>View all suppliers</Link><Link href="/#insights" onClick={closeMenus}>Blog</Link><Link href="/contact" onClick={closeMenus}>Contact</Link></nav>
      <GlobalSearch onOpen={closeMenus}/><Link className={styles.quote} href="/contact#enquiry">Request a Quote <ArrowRight/></Link><button suppressHydrationWarning className={styles.menuButton} onClick={() => setOpen(value => !value)} aria-label="Toggle navigation" aria-expanded={open}>{open ? <X/> : <Menu/>}</button>
