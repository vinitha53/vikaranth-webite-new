...
export default function DetailHeaderClient(){const[open,setOpen]=useState(false);return <><div className={styles.utility}><div className={styles.utilityViewport}><div className={styles.utilityTrack}><UtilitySet/><UtilitySet hidden/></div></div></div><header className={styles.header}><div className={styles.headerInner}><Link className={styles.brand} href="/"><Image src="/logo-vikranth.png" width={190} height={72} alt="Vikranth Chemical Corporation"/></Link><nav className={open?styles.navOpen:""}><Link href="/">Home</Link><Link href="/about">About</Link><Link href="/products">Products</Link><Link href="/industries">Industries</Link><Link href="/associates">Suppliers</Link><Link href="/#insights">Blog</Link><Link href="/contact">Contact</Link></nav><GlobalSearch onOpen={()=>setOpen(false)}/><Link className={styles.quote} href="/contact#enquiry">Request a Quote <ArrowRight/></Link><button className={styles.menuButton} onClick={()=>setOpen(value=>!value)} aria-label="Toggle navigation" aria-expanded={open}>{open?<X/>:<Menu/>}</button></div></header></>}
