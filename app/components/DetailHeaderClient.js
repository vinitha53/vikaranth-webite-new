...
export default function DetailHeaderClient() {
  ...
  useEffect(() => {
    if (!open && !industryMegaOpen && !supplierMegaOpen) return;
    const close = (event) => { if (!headerRef.current?.contains(event.target)) { setIndustryMegaOpen(false); setSupplierMegaOpen(false); } };
    const escape = (event) => { if (event.key === "Escape") { setOpen(false); setIndustryMegaOpen(false); setSupplierMegaOpen(false); } };
    document.addEventListener("pointerdown", close); document.addEventListener("keydown", escape);
    return () => { document.removeEventListener("pointerdown", close); document.removeEventListener("keydown", escape); };
  }, [open, industryMegaOpen, supplierMegaOpen]);
  useEffect(() => {
  ...
  const closeMenus = () => { setOpen(false); setIndustryMegaOpen(false); setSupplierMegaOpen(false); };
  ...
  return <>
    ...
    <header className={styles.header} ref={headerRef}><div className={styles.headerInner}>
      <Link className={styles.brand} href="/"><CocoaMascot/><Image src="/logo-vikranth.webp" width={156} height={73} alt="Vikranth Chemical Corporation"/></Link>
      <nav id="detail-primary-navigation" className={open ? styles.navOpen : ""} aria-label="Primary navigation"><Link aria-current={isCurrent("home") ? "page" : undefined} href="/" onClick={closeMenus}>Home</Link><Link aria-current={isCurrent("about") ? "page" : undefined} href="/about" onClick={closeMenus}>About</Link><Link aria-current={isCurrent("products") ? "page" : undefined} href="/products" onClick={closeMenus}>Products</Link><button suppressHydrationWarning className={`${styles.productsTrigger} ${styles.mobileIndustryTrigger}`} aria-current={isCurrent("industries") ? "page" : undefined} onClick={() => { setIndustryMegaOpen(value => !value); setSupplierMegaOpen(false); }} aria-expanded={industryMegaOpen} aria-controls="site-industries-mega-menu detail-mobile-industries">Industries <ChevronDown/></button><Link className={`${styles.mobileProductsLink} ${styles.mobileIndustryLink}`} aria-current={isCurrent("industries") ? "page" : undefined} href="/industries" onClick={closeMenus}>Industries</Link><div id="detail-mobile-industries" className={`${styles.mobileIndustriesList} ${industryMegaOpen ? styles.mobileIndustriesListOpen : ""}`}>{categories.map((industry) => <Link key={industry.slug} href={industry.href} onClick={closeMenus}>{industry.name}<ArrowRight/></Link>)}<Link className={styles.mobileIndustriesAll} href="/industries" onClick={closeMenus}>View All Industries <ArrowRight/></Link></div><button suppressHydrationWarning className={`${styles.productsTrigger} ${styles.mobileSupplierTrigger}`} aria-current={isCurrent("suppliers") ? "page" : undefined} onClick={() => { setSupplierMegaOpen(value => !value); setIndustryMegaOpen(false); }} aria-expanded={supplierMegaOpen} aria-controls="site-suppliers-mega-menu detail-mobile-suppliers">Suppliers <ChevronDown/></button><Link className={`${styles.mobileProductsLink} ${styles.mobileSupplierLink}`} aria-current={isCurrent("suppliers") ? "page" : undefined} href="/associates" onClick={closeMenus}>Suppliers</Link><div id="detail-mobile-suppliers" className={`${styles.mobileSuppliersList} ${supplierMegaOpen ? styles.mobileSuppliersListOpen : ""}`}>{partners.map((partner) => <Link key={partner.slug} href={`/associates/${partner.slug}`} onClick={closeMenus}><span><img src={partner.logo} alt="" width="150" height="60" loading="lazy" decoding="async"/>{partner.name}</span><ArrowRight/></Link>)}<Link className={styles.mobileSuppliersAll} href="/associates" onClick={closeMenus}>View All Suppliers <ArrowRight/></Link></div><Link aria-current={isCurrent("contact") ? "page" : undefined} href="/contact" onClick={closeMenus}>Contact</Link></nav>
