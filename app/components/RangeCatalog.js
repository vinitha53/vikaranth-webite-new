...
export default function RangeCatalog({ products, indianNames = [], supplierMode = false, categoryField = "usageCategory", mec3Catalog = false }) {
  ...
  const visible = activeCategory === "all" ? inRange : activeCategory === MEC3_FILTER ? [] : inRange.filter(product => categoryFor(product) === activeCategory);
  const visibleGroups = activeCategory === MEC3_FILTER ? [] : activeCategory === "all"
  ...
  return <div className={styles.catalog} data-range-catalog>
    ...
    <div className={styles.categorySection}>
      ...
      <div className={styles.categoryRail} role="group" aria-label={`Filter ${rangeLabel.toLowerCase()} products by category`}>
        ...
        {mec3Catalog && <button className={styles.mec3Filter} type="button" aria-pressed={activeCategory === MEC3_FILTER} onClick={() => setActiveCategory(MEC3_FILTER)}>MEC3 <span>{mec3ProductCount}</span></button>}
      ...
      <div className={styles.resultsSummary} aria-live="polite"><strong>{activeCategory === "all" ? `All ${rangeLabel.toLowerCase()} products` : activeCategory === MEC3_FILTER ? "MEC3 product ranges" : activeCategory}</strong><span>{activeCategory === MEC3_FILTER ? mec3ProductCount : visible.length} {activeCategory === MEC3_FILTER ? "catalogue products" : visible.length === 1 ? "ingredient" : "ingredients"}</span></div>
    </div>
    {activeCategory === MEC3_FILTER && <Mec3CatalogNav />}
