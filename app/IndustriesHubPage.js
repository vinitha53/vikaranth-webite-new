...
export default function IndustriesPage() {
  ...
  return <main className={`${styles.page} ${styles.industryPage}`}>
    ...
    <section className={`${styles.industryHero} ${styles.hubHero}`}>
      <div className={styles.hubMosaic}>{orderedIndustries.slice(0, 4).map((item) => <img key={item.slug} src={item.image} alt="" width="640" height="480" decoding="async" />)}</div>
