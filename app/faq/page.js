...
const faqs = [
  ["Which regions do you supply from Chennai?", "Vikranth supplies food ingredients across South India and pan-India. Share your delivery PIN code so we can confirm freight, lead time and any temperature-controlled handling for the selected product."],
  ...
  ["What information is needed for a quotation?", "Provide the ingredient name, grade or application, required quantity, delivery location, contact information and company name, if applicable."],
...
export default function FaqPage() {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><main className="faq-page"><DetailHeader /><section className="faq-page-hero"><div className="container"><span className="eyebrow">Buyer help centre</span><h1>Food Ingredient Supply Questions</h1><p>Clear answers for manufacturers, bakeries, processors, procurement teams and selected small-quantity buyers.</p></div></section><section className="faq-page-content"><div className="container"><div className="faq-list">{faqs.map(([question,answer]) => <details key={question}><summary><h2>{question}</h2><span aria-hidden="true">+</span></summary><p>{answer}</p></details>)}</div><a className="btn primary" href="/contact/#enquiry">Send Your Requirement <ArrowRight size={16}/></a></div></section><DetailFooter /></main></>;
}
